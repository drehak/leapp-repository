...
class VimMigrate(Actor):
    ...
    def process(self):
      for configs_list in self.consume(VimModel):
        if configs_list == []:
          report_generic(
                          title='Vim configuration files will not be migrated.',
                          summary='No Vim configuration files exists.',
                          severity='low'
                        )
        else:
          ...
          report_generic(
                          ...
                          summary='Changed strings were added into configuration files.',
                          severity='low'
                        )
